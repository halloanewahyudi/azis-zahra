...
<template>
    ...
    <div
        ...
        <div class="w-full">
           <form @submit.prevent="sendMessage" action="" class="flex flex-col gap-5 w-full">
            ...
            <div class="flex flex-col gap-2 w-full">
                <label for="nama">Ucapan / Doa</label>
               <textarea name="" id="" class="input" rows="4" v-model="komen.pesan"></textarea>
            </div>
            <button class="py-3 px-6 rounded-full bg-primary text-light flex items-center gap-2 max-w-max hover:bg-opacity-80">
                Kirim <PlaneVue />
            </button>
           </form>
    ...
    <ul class="max-w-[400px] w-full relative mx-auto p-6  bg-white rounded-xl border-2 border-dashed shadow-xl flex flex-col gap-5">
